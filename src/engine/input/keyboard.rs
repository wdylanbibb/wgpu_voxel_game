...
impl From<winit::event::KeyboardInput> for KeyboardInput {
	fn from(input: winit::event::KeyboardInput) -> Self {
		let f = |virtual_keycode: winit::event::VirtualKeyCode| -> KeyCode { virtual_keycode.into() };

		KeyboardInput {
			scan_code: input.scancode,
			state: input.state.into(),
			key_code: input.virtual_keycode.map(f),
		}
	}
}
...
#[repr(u32)]
// Taken directly from https://github.com/bevyengine/bevy/blob/main/crates/bevy_input/src/keyboard.rs
pub enum KeyCode {
...
// Taken directly from https://github.com/bevyengine/bevy/blob/main/crates/bevy_winit/src/converters.rs
